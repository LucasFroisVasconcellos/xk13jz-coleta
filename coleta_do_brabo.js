// Versão 1.1.4 — última atualização em 2025-07-03T20:03:20Z
...
(function() {
    ...
    // =======================================================================
    const STORAGE_KEY = 'scavenging_automation_state_final_v3';
    ...
    const HTML_PAINEL = `
        <div id="auto-painel-container">
            <h3>Automação de Coleta (v9.2)</h3>
            <div id="config-panel">
    ...
    // =======================================================================
    //  3. LÓGICA DA AUTOMAÇÃO (Com verificação de existência do botão)
    // =======================================================================
    ...
    async function executarColetaUnica(nomeDaColeta, tropasSelecionadas, tempoDesejadoSeg) {
        ...
        if (!cardPai) { return 300; } // Se não achar o card, tenta de novo em 5 minutos

        const botaoComecar = cardPai.querySelector('a.free_send_button');

        // Se o botão NÃO EXISTE, a coleta está em andamento.
        if (!botaoComecar) {
            statusLog.innerHTML += `<div><b>${nomeDaColeta}:</b> Coleta em andamento. Lendo tempo restante...</div>`;
            const tempoRestanteStr = cardPai.querySelector('.return-time')?.textContent.trim() || "0:00:00";
            return parseTempoParaSegundos(tempoRestanteStr);
        }

        // Se o botão EXISTE, procedemos com o cálculo e preenchimento.
        statusLog.innerHTML += `<div><b>${nomeDaColeta}:</b> Pronto para envio. Calculando tropas...</div>`;
    ...
    async function tick() {
            ...
            if (agendamento.ativo) {
                if (agora >= agendamento.proximaExecucao) {
                    ...
                    if (novoEstado[nomeColeta]) {
                        const proximoTimestamp = new Date().getTime() + (tempoRealSeg + 30) * 1000;
        ...
        if(!estadoMudou) statusLog.innerHTML = htmlStatus || "Automação parada. Selecione as coletas e tropas e clique em 'Ligar'.";
    }
